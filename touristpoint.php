...
<style>
    ...
    .touristpoint:hover h1{
        transform: scale(0);
        opacity: 0;
    }

    .touristpoint h1 span{
        position: absolute;
        top:50%;
        left:50%;
    ...
    .touristpoint:hover .description{
        transform: translateY(-100%) scaleY(1);
    }

    .touristpoint .description h2{
        font-size: 12px;
...
<div class="container"><center>
        ...
        foreach(array_filter(glob('*'),'is_dir') as $d){
            echo '<div class="touristpoint">';
            echo ' <img class="thumb" src="touristpoint/'.$d.'/thumb.png">';
            echo ' <h1><span>'.$d.'</span></h1>';
            $txt = OpenFile($d."/description.txt");
            echo '<div class="description"><h2>'.$d.'</h2>'.substr($txt,0,100).' ...</div>';
...
<script>
    ...
    $(".navbar-nav #navTouristPoint").addClass("active");
    if($(window).width()<400){
        $('.touristpoint').css({'margin-left':'0px','margin-right':'0px'});
        var slider = new zsilder("myslider","#moredetails .slider",["img/loading.gif","img/loading.gif"],500/1.9,350/1.9,2000,false);
    }

    function responcive(){
        if($(window).width()>800){
            $('.touristpoint').addClass('notmobile');
        } else {
            $('.touristpoint').removeClass('notmobile');
        }
    }
    responcive();
